...
export type OmitMethod<TShape extends ObjectShape = ObjectShape> = <
...
>(
  ...
  CleanObject<Omit<TSchema['shape'], Exclude<keyof TProps, symbol>>>,
  TSchema['behavior']
>;

export const attachOmit: (attach: CreateObjectFunction) => OmitMethod = (
  attach: typeof createObject,
) => {
  return function (props) {
    const shape: Record<string, Schema> = {};
    for (const [key, value] of Object.entries(this.shape)) {
      if (props[key] !== true && isSchema(value)) {
        shape[key] = value;
      }
    }
